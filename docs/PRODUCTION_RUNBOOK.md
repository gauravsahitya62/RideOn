...
- **production**: separate API + PostgreSQL + Supabase project, explicit CORS, real payment provider credentials, webhook secret, and no in-memory persistence.
...
4. Configure the payment provider webhook URL and secret.
...
- working payment provider + webhook verification
