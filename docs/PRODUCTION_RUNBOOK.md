...
- **production**: separate API + PostgreSQL + Supabase project, explicit CORS, a real payment-provider configuration with verified webhook/verification support, and no in-memory persistence.
...
- `PAYMENT_PROVIDER=<real-supported-provider>`; never `mock`
- provider-specific server-side credentials required by the selected official integration
- provider-specific webhook/signing configuration required by that integration
- `TRUST_PROXY=true` only when the deployment is actually behind a trusted reverse proxy

The API never falls back to mock payments in production. Missing or unimplemented payment-provider configuration is handled as a safe payment error; live payment is not marked successful.
...
4. Configure the selected UPI/payment-provider webhook URL and the provider's official signing/verification secret or credentials.
...
The current repository contains the provider-neutral payment lifecycle and a fail-closed adapter boundary, but no verified live gateway SDK/API implementation. Do not enable production payments until a real provider adapter is installed, its official order/status/refund APIs are implemented, and a staging UPI transaction plus webhook has been verified end-to-end.
