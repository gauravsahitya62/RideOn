...
ALTER TABLE vendors
  ADD CONSTRAINT vendors_service_latitude_range
  CHECK (service_latitude IS NULL OR service_latitude BETWEEN -90 AND 90),
  ADD CONSTRAINT vendors_service_longitude_range
...
ALTER TABLE bookings
  ADD CONSTRAINT bookings_delivery_latitude_range
  CHECK (delivery_latitude IS NULL OR delivery_latitude BETWEEN -90 AND 90),
  ADD CONSTRAINT bookings_delivery_longitude_range
  CHECK (delivery_longitude IS NULL OR delivery_longitude BETWEEN -180 AND 180),
  ADD CONSTRAINT bookings_vendor_service_latitude_range
  CHECK (vendor_service_latitude IS NULL OR vendor_service_latitude BETWEEN -90 AND 90),
  ADD CONSTRAINT bookings_vendor_service_longitude_range
  CHECK (vendor_service_longitude IS NULL OR vendor_service_longitude BETWEEN -180 AND 180),
  ADD CONSTRAINT bookings_route_distance_nonnegative
  CHECK (route_distance_meters IS NULL OR route_distance_meters >= 0),
  ADD CONSTRAINT bookings_route_duration_nonnegative
