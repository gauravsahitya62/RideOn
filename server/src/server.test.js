...
test('UPI payment creation persists pending state for an authenticated booking', async () => {
  ...
  const state=await request('/api/v1/bookings/'+booking.bookingId,{headers:{authorization:'Bearer '+login.accessToken}});
  const statePayload=await state.json();
  assert.equal(statePayload.booking.paymentStatus,'pending');
