...
app.post('/api/v1/fleet-ops/bookings/:id/delivery/start',supabaseRequireAuth,requireFleetOps,async(req,res)=>{try{const result=await repository.startDelivery(req.user.id,req.params.id,{actorRole:req.user.role});res.json({booking:result.booking,session:result.session});}catch(error){res.status(error?.code==='FORBIDDEN'?403:409).json({error:{code:error?.code||'DELIVERY_START_FAILED',message:error?.message||'Delivery could not be started.'}});}});
app.post('/api/v1/fleet-ops/bookings/:id/delivery/location',supabaseRequireAuth,requireFleetOps,async(req,res)=>{const p=z.object({latitude:z.coerce.number().min(-90).max(90),longitude:z.coerce.number().min(-180).max(180),accuracy:z.coerce.number().min(0).max(10000).nullable().optional(),timestamp:z.string().datetime().optional()}).safeParse(req.body||{});if(!p.success)return res.status(400).json({error:{code:'INVALID_DELIVERY_LOCATION',message:'Please provide a valid delivery location.'}});try{const session=await repository.updateDeliveryLocation(req.user.id,req.params.id,p.data);res.json({session});}catch(error){res.status(error?.code==='FORBIDDEN'?403:409).json({error:{code:error?.code||'DELIVERY_LOCATION_FAILED',message:error?.message||'Delivery location could not be updated.'}});}});
app.post('/api/v1/fleet-ops/bookings/:id/delivery/complete',supabaseRequireAuth,requireFleetOps,async(req,res)=>{const p=z.object({latitude:z.coerce.number().min(-90).max(90).nullable().optional(),longitude:z.coerce.number().min(-180).max(180).nullable().optional()}).safeParse(req.body||{});if(!p.success)return res.status(400).json({error:{code:'INVALID_DELIVERY_LOCATION',message:'Please provide a valid final delivery location.'}});try{const result=await repository.completeDelivery(req.user.id,req.params.id,{...p.data,actorRole:req.user.role});const booking=await repository.transitionRentalLifecycle({bookingId:req.params.id,nextState:'READY_FOR_PICKUP',actorUserId:req.user.id,actorRole:req.user.role,note:'delivery_completed'});res.json({booking,session:result.session});}catch(error){res.status(error?.code==='FORBIDDEN'?403:409).json({error:{code:error?.code||'DELIVERY_COMPLETE_FAILED',message:error?.message||'Delivery could not be completed.'}});}});
app.post('/api/v1/fleet-ops/bookings/:id/delivery/abort',supabaseRequireAuth,requireFleetOps,async(req,res)=>{try{const result=await repository.abortDelivery(req.user.id,req.params.id,{actorRole:req.user.role});res.json(result);}catch(error){res.status(error?.code==='FORBIDDEN'?403:409).json({error:{code:error?.code||'DELIVERY_ABORT_FAILED',message:error?.message||'Delivery could not be aborted.'}});}});
...
app.post('/api/v1/fleet-ops/bookings/:id/damage-review',supabaseRequireAuth,requireRole('admin','support'),async(req,res)=>{const p=z.object({caseId:z.string().uuid(),status:z.enum(['reported','under_review','approved','disputed','resolved','rejected']),approvedDeductionPaise:z.number().int().min(0).optional(),notes:z.string().max(2000).nullable().optional()}).safeParse(req.body||{});if(!p.success)return res.status(400).json({error:{code:'VALIDATION_ERROR',message:'Invalid damage review.'}});try{const damageCase=await repository.updateFleetDamageCase({caseId:p.data.caseId,bookingId:req.params.id,actorUserId:req.user.id,status:p.data.status,approvedDeductionPaise:p.data.approvedDeductionPaise,notes:p.data.notes});res.json({damageCase});}catch(error){const status=['FORBIDDEN'].includes(error?.code)?403:['DAMAGE_CASE_NOT_FOUND'].includes(error?.code)?404:409;res.status(status).json({error:{code:error?.code||'DAMAGE_REVIEW_FAILED',message:error?.message||'Damage review could not be updated.'}});}});

app.post('/api/v1/fleet-ops/bookings/:id/deposit/settle',supabaseRequireAuth,requireFleetOps,async(req,res)=>{const p=z.object({deductionPaise:z.number().int().min(0).default(0),reason:z.string().max(2000).optional().default(''),evidenceReference:z.string().max(500).optional().default(''),refundProviderReference:z.string().max(255).optional().default('')}).safeParse(req.body||{});if(!p.success)return res.status(400).json({error:{code:'VALIDATION_ERROR',message:'Invalid deposit settlement details.',details:p.error.flatten()}});try{const result=await repository.settleFleetSecurityDeposit({bookingId:req.params.id,actorUserId:req.user.id,...p.data});res.json(result);}catch(error){const status=['DEPOSIT_ALREADY_SETTLED','DEPOSIT_SETTLEMENT_NOT_ALLOWED','DEPOSIT_DEDUCTION_INVALID','DEPOSIT_DEDUCTION_DOCUMENTATION_REQUIRED','DAMAGE_REVIEW_NOT_APPROVED'].includes(error?.code)?409:error?.code==='FORBIDDEN'?403:404;res.status(status).json({error:{code:error?.code||'DEPOSIT_SETTLEMENT_FAILED',message:error?.message||'Security deposit settlement could not be completed.'}});}});
app.post('/api/v1/fleet-ops/bookings/:id/deposit-settlement',supabaseRequireAuth,requireFleetOps,async(req,res)=>{return res.redirect(307,`/api/v1/fleet-ops/bookings/${encodeURIComponent(req.params.id)}/deposit/settle`);});
