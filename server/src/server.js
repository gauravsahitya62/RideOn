...
const baseBookingSchema = z.object({
  ...
  vehicleId: z.string().min(1), startAt: z.string().datetime(), endAt: z.string().datetime(),
  delivery: z.boolean().default(true), address: z.string().trim().min(8).max(300), notes: z.string().max(500).optional(),
...
function normalizeBookingInput(body = {}) {
  ...
  const start = typeof body.startDate === 'string' ? new Date(`${body.startDate}T10:00:00.000Z`) : null;
  if (!start || Number.isNaN(start.getTime())) return body;
...
function serializeBooking(booking) {
  return { ...booking, bookingId: booking.id, vehicleName: booking.vehicle.name, totalPrice: booking.pricing.total, total: booking.pricing.total };
}
...
app.get('/api/v1/vehicles', (req, res) => {
  ...
  const data = fleet.filter((v) => v.active && (!type || type === 'all' || v.type === type) && (!city || v.city.toLowerCase() === city) && (!q || `${v.name} ${v.subtitle}`.toLowerCase().includes(q)));
...
app.post('/api/v1/bookings/quote', (req, res) => {
  ...
  const data = pricing(vehicle, parsed.data.startAt, parsed.data.endAt, parsed.data.delivery);
  res.json({ data: { vehicleId: vehicle.id, ...data, disclaimer: 'Demo estimate; availability and final fees must be confirmed.' }, quote: { vehicleId: vehicle.id, ...data } });
});
app.post('/api/v1/bookings', (req, res) => {
  const normalized = normalizeBookingInput(req.body);
  const parsed = baseBookingSchema.safeParse(normalized);
  ...
  res.status(201).json({ data, booking: data });
});
app.get('/api/v1/bookings/:id', (req, res) => {
  const booking = bookings.get(req.params.id);
  if (!booking) return res.status(404).json({ error: { code: 'BOOKING_NOT_FOUND' } });
  const data = serializeBooking(booking); res.json({ data, booking: data });
});
app.patch('/api/v1/bookings/:id/cancel', (req, res) => {
  const booking = bookings.get(req.params.id);
  if (!booking) return res.status(404).json({ error: { code: 'BOOKING_NOT_FOUND' } });
  if (!['requested', 'confirmed'].includes(booking.status)) return res.status(409).json({ error: { code: 'CANNOT_CANCEL', message: 'This booking can no longer be cancelled through this endpoint' } });
  booking.status = 'cancelled'; booking.updatedAt = new Date().toISOString();
  const data = serializeBooking(booking); res.json({ data, booking: data });
});
