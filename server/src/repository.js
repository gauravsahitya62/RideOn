...
export function createRepository({ databaseUrl, fleet }) {
  ...
  const RENTAL_LIFECYCLE = new Set(['CONFIRMED','DELIVERY_ASSIGNED','PICKUP_ASSIGNED','DELIVERY_STARTED','READY_FOR_PICKUP','HANDED_OVER','ACTIVE_RENTAL','RETURN_REQUESTED','RETURNED','INSPECTION','COMPLETED','DAMAGE_REVIEW_REQUIRED','OVERDUE']);
  const RENTAL_TRANSITIONS={CONFIRMED:new Set(['DELIVERY_ASSIGNED','PICKUP_ASSIGNED','READY_FOR_PICKUP']),DELIVERY_ASSIGNED:new Set(['DELIVERY_STARTED','READY_FOR_PICKUP']),PICKUP_ASSIGNED:new Set(['READY_FOR_PICKUP','DELIVERY_STARTED']),DELIVERY_STARTED:new Set(['HANDED_OVER']),READY_FOR_PICKUP:new Set(['HANDED_OVER']),HANDED_OVER:new Set(['ACTIVE_RENTAL']),ACTIVE_RENTAL:new Set(['RETURN_REQUESTED','OVERDUE']),OVERDUE:new Set(['RETURN_REQUESTED','RETURNED']),RETURN_REQUESTED:new Set(['RETURNED']),RETURNED:new Set(['INSPECTION']),INSPECTION:new Set(['COMPLETED','DAMAGE_REVIEW_REQUIRED']),DAMAGE_REVIEW_REQUIRED:new Set(['COMPLETED','INSPECTION']),COMPLETED:new Set([])};
  ...
  const normalizeRentalLifecycle=(booking)=>String(booking?.lifecycleState??booking?.lifecycle_state??(booking?.status==='in_progress'?'ACTIVE_RENTAL':booking?.status==='completed'?'COMPLETED':'CONFIRMED')).toUpperCase();
  ...
  async function transitionRentalLifecycle({bookingId,nextState,actorUserId,actorRole,note=null}={}) {
    ...
    if(!['admin','support','delivery_staff'].includes(String(actorRole||'').toLowerCase()))throw fleetLifecycleError('Fleet operations access is required.','FORBIDDEN');
    if(!useDatabase){const b=memory.bookings.get(String(bookingId));if(!b)throw fleetLifecycleError('Booking not found.','BOOKING_NOT_FOUND');const current=normalizeRentalLifecycle(b);if(current!==state&&!RENTAL_TRANSITIONS[current]?.has(state))throw fleetLifecycleError('Rental lifecycle transition is not allowed.','INVALID_RENTAL_TRANSITION');if(state==='ACTIVE_RENTAL'&&!FLEET_FINANCIAL_READY.has(String(b.paymentStatus)))throw fleetLifecycleError('Payment must be confirmed before the rental can start.','PAYMENT_NOT_CONFIRMED');if(state==='COMPLETED'&&!['INSPECTION','DAMAGE_REVIEW_REQUIRED'].includes(current))throw fleetLifecycleError('Booking cannot be completed before inspection.','COMPLETION_NOT_ALLOWED');b.lifecycleState=state;b.updatedAt=new Date().toISOString();return b;}
    const client=await pool.connect();try{await client.query('begin');const q=await client.query('select id,lifecycle_state,vehicle_id,payment_status from bookings where id=$1 for update',[bookingId]);const b=q.rows[0];if(!b)throw fleetLifecycleError('Booking not found.','BOOKING_NOT_FOUND');const current=String(b.lifecycle_state||'CONFIRMED').toUpperCase();if(current!==state&&!RENTAL_TRANSITIONS[current]?.has(state))throw fleetLifecycleError('Rental lifecycle transition is not allowed.','INVALID_RENTAL_TRANSITION');if(state==='ACTIVE_RENTAL'&&!FLEET_FINANCIAL_READY.has(String(b.payment_status)))throw fleetLifecycleError('Payment must be confirmed before the rental can start.','PAYMENT_NOT_CONFIRMED');if(state==='COMPLETED'&&!['INSPECTION','DAMAGE_REVIEW_REQUIRED'].includes(current))throw fleetLifecycleError('Booking cannot be completed before inspection.','COMPLETION_NOT_ALLOWED');await client.query('update bookings set lifecycle_state=$2,updated_at=now() where id=$1',[bookingId,state]);await client.query("insert into fleet_operation_audit(vehicle_id,booking_id,actor_user_id,action,previous_state,next_state,details) values($1,$2,$3,'lifecycle_transition',$4,$5,$6)",[b.vehicle_id,bookingId,actorUserId||null,current,state,JSON.stringify({actorRole,note})]);await client.query('commit');return getBooking(bookingId);}catch(e){try{await client.query('rollback')}catch{}finally{client.release();}throw e;}
  }

  async function requestRentalReturn({bookingId,customerId,returnLocation=null,notes=null}={}) {
    if(!useDatabase){const b=memory.bookings.get(String(bookingId));if(!b||String(b.customerId)!==String(customerId))throw fleetLifecycleError('Booking not found.','BOOKING_NOT_FOUND');const v=b.vehicle||memory.vehicles.get(String(b.vehicleId));if(!['ACTIVE_RENTAL','OVERDUE'].includes(normalizeRentalLifecycle(b))||String(v?.operationalState||'')!=='RENTED')throw fleetLifecycleError('Rental is not active.','RETURN_NOT_ALLOWED');b.lifecycleState='RETURN_REQUESTED';b.returnRequestedAt=new Date().toISOString();b.returnLocation=returnLocation;b.returnNotes=notes;return b;}
    const client=await pool.connect();try{await client.query('begin');const q=await client.query('select b.id,b.customer_id,b.vehicle_id,b.lifecycle_state,v.operational_state from bookings b join vehicles v on v.id=b.vehicle_id where b.id=$1 and b.customer_id=$2 for update',[bookingId,customerId]);const b=q.rows[0];if(!b)throw fleetLifecycleError('Booking not found.','BOOKING_NOT_FOUND');if(!['ACTIVE_RENTAL','OVERDUE'].includes(String(b.lifecycle_state||''))||String(b.operational_state)!=='RENTED')throw fleetLifecycleError('Rental is not active.','RETURN_NOT_ALLOWED');await client.query("update bookings set lifecycle_state='RETURN_REQUESTED',return_requested_at=now(),return_location=$2,return_notes=$3,updated_at=now() where id=$1",[bookingId,returnLocation,notes]);await client.query("insert into fleet_operation_audit(vehicle_id,booking_id,actor_user_id,action,previous_state,next_state,details) values($1,$2,$3,'return_requested',$4,'RETURN_REQUESTED',$5)",[b.vehicle_id,bookingId,customerId,b.lifecycle_state,JSON.stringify({returnLocation,notes})]);await client.query('commit');return getBooking(bookingId,customerId);}catch(e){try{await client.query('rollback')}catch{}finally{client.release();}throw e;}
  }
