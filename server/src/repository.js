...
export function createRepository({ databaseUrl, fleet }) {
  ...
  async function createFleetOrder({customerId,vehicleIds,startAt,endAt,delivery=true,address='',deliveryLatitude=null,deliveryLongitude=null,idempotencyKey=null}={}) {
    ...
    try{
      ...
      await client.query(`insert into fleet_order_items(fleet_order_id,booking_id,vehicle_id,line_rental_total_paise,line_delivery_fee_paise,line_platform_fee_paise,line_security_deposit_paise,line_total_paise) select $1,id,vehicle_id,rental_total_paise,delivery_fee_paise,platform_fee_paise,tax_paise,discount_paise,security_deposit_paise,total_paise from bookings where fleet_order_id=$1`,[orderId]);
      await client.query('commit');
