...
const C={ink:'#111827',muted:'#6B7280',orange:'#E56A3D',bg:'#F7F5F1',white:'#FFFFFF',line:'#E7E2DA',green:'#218765'};
const isExpoGo=Constants.appOwnership==='expo';

const CITY_CENTERS={
  udaipur:{latitude:24.5854,longitude:73.7125},
};
const DEFAULT_ZOOM={latitudeDelta:0.16,longitudeDelta:0.16};
...
export default function MapsDiscoveryScreen({city,onBack,onBookVehicle}){
  ...
  const loadVendors=useCallback(async()=>{
    ...
    try{
      ...
      const next=Array.isArray(result?.vendors)?result.vendors:Array.isArray(result?.data)?result.data:[];setVendors(next);const points=next.map(v=>({latitude:Number(v.latitude),longitude:Number(v.longitude)})).filter(p=>Number.isFinite(p.latitude)&&Number.isFinite(p.longitude));if(points.length){const lat=points.reduce((sum,p)=>sum+p.latitude,0)/points.length;const lon=points.reduce((sum,p)=>sum+p.longitude,0)/points.length;setMapRegion({...DEFAULT_ZOOM,latitude:lat,longitude:lon});}
    }catch(e){
      setError('RideOn service coverage is temporarily unavailable. Please retry.');
  ...
  const cityRegion=useMemo(()=>defaultRegionForCity(city),[city]);
  const [mapRegion,setMapRegion]=useState(cityRegion);
  ...
  return <View style={styles.root}>
    <View style={styles.top}><TouchableOpacity onPress={onBack} style={styles.back}><Text style={styles.backGlyph}>‹</Text></TouchableOpacity><View><Text style={styles.kicker}>RIDEON SERVICE AREA</Text><Text style={styles.title}>RideOn in {city}</Text></View><View style={{width:42}}/></View>
    <View style={styles.mapWrap}>
      {loading?<View style={styles.mapState}><ActivityIndicator color={C.orange}/><Text style={styles.muted}>Loading RideOn service locations…</Text></View>:
      ...
      <View style={styles.mapHint}><Text style={styles.mapHintText}>Tap a RideOn location to see available vehicles. Tap the map to set a delivery point.</Text></View>
    ...
    <View style={styles.sheet}>
      ...
      </View>:<View style={styles.empty}><Text style={styles.sectionTitle}>Select a RideOn location</Text><Text style={styles.muted}>Choose a service marker to browse the vehicles available in this area.</Text></View>}
      {selectedVendor&&<View style={styles.vehicles}>
        ...
        {vehicleLoading?<Text style={styles.muted}>Loading vehicles…</Text>:
          ...
          <Text style={styles.muted}>No active RideOn vehicles are currently available at this location.</Text>}
      </View>}
    </View>
  </View>;
}
