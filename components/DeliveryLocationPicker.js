...
const DEFAULT_ZOOM={latitudeDelta:0.12,longitudeDelta:0.12};
...
export default function DeliveryLocationPicker({value,onChange,city}){
  ...
  const [mapRegion,setMapRegion]=useState(
    currentPoint
      ? {...currentPoint,latitudeDelta:.05,longitudeDelta:.05}
      : defaultRegionForCity(city)
  );

  useEffect(()=>{
    if(currentPoint){
      setMapRegion({...currentPoint,latitudeDelta:.05,longitudeDelta:.05});
    }else{
      setMapRegion(defaultRegionForCity(city));
    }
  },[city,currentPoint]);
